...
class OneHot_Encoding(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        print(
            f" .... INSIDE PIPELINE: Doing Onehot Encoding of {self.variables_to_convert}: "
        )
        X = X.copy()
        for field in self.variables_to_convert:
            if X[field].dtype == "category":
                enc = OneHotEncoder(
                    sparse_output=False, handle_unknown="ignore", max_categories=None
                )
...
class Categorical_to_Num(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        print(
            f" .... INSIDE PIPELINE: Doing Categorical to NUM Encoding of {self.variables_to_convert}: "
        )
        X = X.copy()
        for field in self.variables_to_convert:
            if X[field].dtype == "category":
                ...
                pd.DataFrame(le.classes_, columns=[f"{field}_cat"]).to_csv(
                    pathfile, index=False
                )
            else:
                print(f"{field } variable must be category. NOT TRASFORMED")
        return X
...
class Convert_Str_to_Float(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        print(
            f" .... INSIDE PIPELINE: Converting Strings to float of {self.variables_to_convert}: "
        )
        X = X.copy()
        for field in self.variables_to_convert:
...
class Make_Categorical(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        print(
            f" .... INSIDE PIPELINE: Converting to categories: {self.variables_to_categorize}: "
        )
        X = X.copy()
...
class Standarize_Text(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        print(
            f" .... INSIDE PIPELINE: Standarizing to lower and _ of: {self.variables_to_standarize_text}: "
        )
        X = X.copy()
...
class KeepNecessaryFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        print(
            f" .... INSIDE PIPELINE: Dropping no needed vars, keeping only: {self.variables_to_keep}"
        )
        X = X.copy()
...
class DropUnecessaryFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        print(
            f" .... INSIDE PIPELINE: Dropping this vars after transformed: {self.variables_to_drop}: "
        )
        X = X.copy()
