...
def datapipeline3(cfg: DictConfig) -> Pipeline:
    ...
    datapipeline = Pipeline(
        [
            ...
            ("fulfill_NAN", pp.Fullfill_NAN()),
            (
                "standarize_text",
                pp.Standarize_Text(
                    variables_to_standarize_text=cfg.data_pipeline.standarize_text
                ),
            ),
            (
                "make_categorical",
                pp.Make_Categorical(
                    variables_to_categorize=cfg.data_pipeline.to_categorize
                ),
            ),
            (
                "convert_str_float",
                pp.Convert_Str_to_Float(
                    variables_to_convert=cfg.data_pipeline.str_to_float
                ),
            ),
            (
                "categorical_to_num",
                pp.Categorical_to_Num(
                    variables_to_convert=cfg.data_pipeline.category_to_num,
            ...
            (
                "onehot_encoding",
                pp.OneHot_Encoding(
                    variables_to_convert=cfg.data_pipeline.onehot_encoding
                ),
            ),
            (
                "drop_fields",
                pp.DropUnecessaryFeatures(
                    variables_to_drop=cfg.data_pipeline.drop_features
                ),
            ),
        ...
        [
            (
                ...
                pp.CategoricalToNumerical(
                    variables=cfg.data_pipeline.numerical_vars_from_numerical
                ),
            ),
            ...
            (
                "label extraction",
                pp.LabelExtraction(
                    variables=cfg.data_pipeline.categorical_label_extraction
                ),
            ),
            (
                "rare label encoder",
                pp.RareLabelCategoricalEncoder(
                    tol=0.01, variables=cfg.data_pipeline.categorical_vars
                ),
            ),
            (
                "categorical_encoder",
