...
def main(cfg: DictConfig) -> None:
    pathfile_l = os.path.join(
        cfg.paths.processed_data_dir, cfg.file_names.processed_data
    )
    data = load_data(
        pathfile_l,
    ...
    pathfile_train_futures = os.path.join(
        cfg.paths.processed_data_dir, cfg.file_names.train_features
    )
    pathfile_train_labels = os.path.join(
        cfg.paths.processed_data_dir, cfg.file_names.train_labels
    )
    pathfile_validation_futures = os.path.join(
        cfg.paths.processed_data_dir, cfg.file_names.validation_features
    )
    pathfile_validation_labels = os.path.join(
        cfg.paths.processed_data_dir, cfg.file_names.validation_labels
    )
    pathfile_test_futures = os.path.join(
        cfg.paths.processed_data_dir, cfg.file_names.test_features
    )
    pathfile_test_labels = os.path.join(
        cfg.paths.processed_data_dir, cfg.file_names.test_labels
    )
