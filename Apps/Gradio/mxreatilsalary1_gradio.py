...
def main(cfg: DictConfig) -> None:
    ...
    def prediction(state, income, employees):
        input = pd.DataFrame(
            [[state, float(income), int(employees)]], columns=experiment.feature_list
        )
    ...
    iface = gr.Interface(
        ...
        inputs=[
            gr.inputs.Dropdown(
                label="Estado Mx",
                choices=experiment.catalogues["state"],
                default="Morelos",
            ),
            gr.inputs.Slider(
                minimum=4000, maximum=10000, step=200, label="Ventas diarias estimadas"
            ),
            gr.inputs.Slider(
                minimum=4, maximum=8, step=1, label="Empleados en el local"
            ),
        ],
